...
#
# == NOTE:
# Loose standard for default rule naming (most specific to least):
#   0_default = specific rules to be caught early (ex. matching on programname +
#               error + etc.)
#   1_default = specific rules that have a corresponding "0_default" entry but
  ...
  if empty($server_conf) {
    ...
    if $use_default_spool_rules {
      rsyslog::rule::local { '0_default_spool':
        rule      => 'if ($syslogfacility-text == \'uucp\' or ($syslogfaciliy-text == \'news\' and $syslogpriority-text == \'crit\')) then',
