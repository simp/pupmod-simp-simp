...
#
# @param rsync_base [AbsolutePath] The path to the beginning of the rsync space
#   for this system. There must be a directory per environmet that you want to
#   serve to clients.
#
# @param use_stunnel [Boolean] If set, hosts_allow will be set to ``127.0.0.1``
#   so that the stunnel'd rsync will be used.
#
# @param hosts_allow [NetList] The hosts from which to allow access to the
#   rsync shares. This ootion has no effect if ``$use_stunnel`` is ``true``.
#
# @author Trevor Vaughan <tvaughan@onyxpoint.com>
#
class simp::server::rsync_shares (
  $rsync_base  = '/var/simp/rsync/environments',
  $use_stunnel = defined('$::use_stunnel') ? { true => $::use_stunnel, default => lookup('rsync::server::use_stunnel', { 'default_value' => true }) },
  ...
  simp_server_rsync_base($rsync_base).each |String $_env| {
