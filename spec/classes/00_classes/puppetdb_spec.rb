      ...
      context "on #{os}" do
        if os_facts[:kernel] == 'windows'
          ...
          context 'with use_puppet_ssl_certs => false and manage_puppetserver => false' do
            ...
            it { is_expected.to compile.with_all_deps }
            it { is_expected.to contain_class('puppetdb::master::puppetdb_conf') }
            it { is_expected.not_to contain_class('puppetdb::master::puppetdb_conf').that_notifies('Class[pupmod::master::base]') }
            it { is_expected.not_to contain_class('pupmod::master::base') }
          end
          ...
          context 'with puppetdb::master::config::manage_config: false' do
            ...
            it { is_expected.to compile.with_all_deps }
            it { is_expected.not_to contain_file("#{Puppet[:confdir]}/puppetdb.conf") }
            it { is_expected.not_to contain_class('pupmod::master::base') }
          end
        end
      end
