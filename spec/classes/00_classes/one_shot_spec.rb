      ...
      context "on #{os}" do
        ...
        if os_facts[:kernel] == 'windows'
          ...
          context 'with no params' do
            it { expect { is_expected.to compile.with_all_deps }.to raise_error(%r{must specify either.+user_password.+user_ssh_authorized_key}) }
          end
          ...
          context 'when user management disabled' do
            ...
            it { is_expected.not_to create_file("/var/local/#{username}") }
            it { is_expected.not_to create_pam__access__rule("allow_#{username}") }
            it { is_expected.not_to create_sudo__user_specification(username) }
          end
        end
      end
    end
