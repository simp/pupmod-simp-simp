...
EOF"
          ...
          # Ensure the sssd cache is built, don't wait for enum timeout
          if os_release == '7' then
            on node, "systemctl restart sssd"
          elsif os_release.to_f < 7 and fact_on(node, 'operatingsystemrelease').to_f >= 6.7 then
