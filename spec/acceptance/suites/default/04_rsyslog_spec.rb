  ...
  context 'with is_server = true' do
    ...
    it 'testing default log_server rules' do
      servers.each do |server|
        ...
        # NOTE: see logger man page for potential kern issues
        # NOTE: on server, "IPT does not work!"
        ...
        on server, "logger -p mail.none -t mail LOGGERMAILNONE"
        on server, "logger -p authpriv.none -t authpriv LOGGERAUTHPRIVNONE"
        on server, "logger -p cron.none -t cron LOGGERCRONNONE"
        on server, "logger -p local6.none -t local6 LOGGERLOCAL6NONE"
        on server, "logger -p local5.none -t local5 LOGGERLOCAL5NONE"
        ...
        test_array = [
          ...
          ["LOGGERMAILNONE", "maillog.log"],
          ["LOGGERAUTHPRIVNONE", "secure.log"],
          ["LOGGERCRONNONE", "cron.log"],
          ["LOGGERLOCAL6NONE", "secure.log"],
          ["LOGGERLOCAL5NONE", "secure.log"]]
    ...
    # Only rules previously caught by 0/1 will be tested so there is no overlap.
    #
    #
    ...
    it 'testing log_server with 0/1 stop rules disabled' do
      servers.each do |server|
        ...
        on server, "logger -p cron.none -t cron LOGGERCRONNONESECURE"
        on server, "logger -p local7.warn -t boot BOOTSECURE"
        on server, "logger -p cron.emerg -t cron LOGGEREMERGSECURE"

        test_array = ["LOGGERSUDOSHSECURE","LOGGERCRONNONESECURE",
    ...
    # Only rules previously caught by 0/1/7 will be tested so there is no overlap.
    #
    #
    ...
    it 'testing log_server with 0/1/7 stop rules disabled' do
      servers.each do |server|
        on server, "logger -p mail.none -t mail LOGGERMAILNONEMESSAGES"
        on server, "logger -p authpriv.none -t authpriv LOGGERAUTHPRIVNONEMESSAGES"
        on server, "logger -p cron.none -t cron LOGGERCRONNONEMESSAGES"
        on server, "logger -p local6.none -t local6 LOGGERLOCAL6NONEMESSAGES"
        on server, "logger -p local5.none -t local5 LOGGERLOCAL5NONEMESSAGES"
